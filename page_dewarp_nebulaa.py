...
TEXT_MIN_HEIGHT = 3      # min reduced px height of detected text contour
TEXT_MIN_ASPECT = 1.5    # filter out text contours below this w/h ratio
TEXT_MAX_THICKNESS = 20#10  # max reduced px thickness of detected text contour

EDGE_MAX_OVERLAP = 1.0   # max reduced px horiz. overlap of contours in span
EDGE_MAX_LENGTH = 70.0 #100.00  # max reduced px length of edge connecting contours
EDGE_ANGLE_COST = 5.0   # cost of angles in edges (tradeoff vs. length)
EDGE_MAX_ANGLE = 4.0     # maximum change in angle allowed between contours
...
SPAN_PX_PER_STEP = 20#20    # reduced px spacing for sampling along spans
...
def resize_to_screen(src, mask, copy=False):
    ...
    if scl > 1.0:
        ...
        mask = cv2.resize(mask, (0, 0), None, inv_scl, inv_scl, cv2.INTER_AREA)
    elif copy:
        img = src.copy()
    else:
        img = src
...
def get_contours(mask, name, small ,pagemask, masktype):
    ...
    # cv2.imwrite('contours.jpg',small)
    # if DEBUG_LEVEL >= 2:
    ...
    for span in spans:
        ...
        for cinfo in span:
            ...
            contour_points += [(x+xmin, means[x]+ymin)
                               for x in range(int(start), int(len(means)), int(step))]
...
def remap_image(name, coloured_orig, small, page_dims, params):
    height = 0.5 * page_dims[1] * OUTPUT_ZOOM * coloured_orig.shape[0]
    ...
    if height > 4000 or width > 4000:
        return coloured_orig


    # print('original ratio {}'.format(coloured_orig.shape[0]/coloured_orig[1]))
    # print( '  output will be {}x{}'.format(width, height))
    # print('after dewarp ratio {}'.format(height/width))
    ...
    image_points = norm2pix(coloured_orig.shape, image_points, False)
    ...
    coloured_remappped = cv2.remap(coloured_orig, image_x_coords, image_y_coords,
                          cv2.INTER_CUBIC,
                          None, cv2.BORDER_REPLICATE)

    # coloured_remappped = imutils.resize(coloured_remappped, width = 1800)

    cv2.imwrite('coloured_remappped.jpg',coloured_remappped)

 
    return coloured_remappped




def page_dewarp( coloured_orig, text_mask):

    global REMAP_DECIMATE
    ...
    TEXT_MIN_WIDTH = 15 #15      # min reduced px width of detected text contour
    ...
    visual_small, text_mask = resize_to_screen(coloured_orig, text_mask)
    ...
    if DEBUG_LEVEL >= 3:
        debug_show(name, 0.0, 'original', visual_small)

    small_with_bor = cv2.copyMakeBorder(visual_small,BORDER,BORDER,BORDER,BORDER,  cv2.BORDER_REPLICATE, None, (255,255,255))
    text_mask = cv2.copyMakeBorder(text_mask, BORDER,BORDER,BORDER,BORDER,  cv2.BORDER_CONSTANT, None, (0,0,0))

    pagemask, page_outline = get_page_extents(small_with_bor)

    cinfo_list = get_contours(text_mask, name, visual_small, pagemask, 'text')

    visual_small = cv2.copyMakeBorder(visual_small, BORDER,BORDER,BORDER,BORDER,  cv2.BORDER_REPLICATE, None, (255,255,255))
    small = cv2.copyMakeBorder(visual_small, BORDER,BORDER,BORDER,BORDER,  cv2.BORDER_REPLICATE, None, (255,255,255))

    spans = assemble_spans(name, visual_small, pagemask, cinfo_list)

    if len(spans) < 3:
        # print( '  detecting lines because only', len(spans), 'text spans')
        cinfo_list = get_contours(text_mask, name, small, pagemask, 'line')
        spans2 = assemble_spans(name, visual_small, pagemask, cinfo_list)
        if len(spans2) > len(spans):
            spans = spans2

    if len(spans) <= 1:
        # print( 'skipping', name, 'because only', len(spans), 'spans')
        return clean_unskew_thresh, coloured_orig
        # continue
    ...
    if sum([len(pts) for pts in span_points]) < span_thresh :
        return coloured_orig




    corners, ycoords, xcoords = keypoints_from_samples(name, visual_small,
                                                       pagemask,
                                                       page_outline,
                                                       span_points)
    ...
    params = optimize_params(name, visual_small,
                             dstpoints,
                             span_counts, params)
    ...
    # try:
    coloured_outfile = remap_image(name, coloured_orig, small, page_dims, params)
    # except:
    #     return coloured_orig


    return  coloured_outfile
